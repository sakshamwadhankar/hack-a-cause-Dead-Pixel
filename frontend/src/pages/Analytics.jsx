...
<html>
<head>
  <title>JalRakshak Water Stress Report</title>
  <style>
...
<body>
  <div class="header">
    <h1>💧 JalRakshak Water Stress Report</h1>
    <div class="date">
      <strong>District:</strong> ${district || 'All India'} ${state ? `(${state})` : ''}<br>
      <strong>Generated:</strong> ${new Date().toLocaleString('en-IN')}<br>
      <strong>Total Villages:</strong> ${villages.length}
    </div>
  </div>
  ...
  <table>
    ...
    <tr>
      <td>Critical Villages</td>
      <td class="critical">${stats?.critical_count || 0}</td>
    </tr>
    <tr>
      <td>High Risk Villages</td>
      <td class="high">${stats?.high_count || 0}</td>
    </tr>
    <tr>
      <td>Moderate Risk Villages</td>
      <td class="moderate">${stats?.moderate_count || 0}</td>
    </tr>
    <tr>
      <td>Safe Villages</td>
      <td class="safe">${stats?.safe_count || 0}</td>
    </tr>
    <tr>
      <td>Average WSI Score</td>
      <td>${stats?.avg_wsi?.toFixed(1) || 'N/A'}</td>
    </tr>
    <tr>
      <td>Population at Risk</td>
      <td>${getTotalAtRiskPopulation().toLocaleString()}</td>
    </tr>
  </table>
    ...
    <tbody>
      ${villages.map(v => `
        <tr>
          ...
          <td>${v.water_stress_index.toFixed(1)}</td>
          <td class="${v.stress_level}">${v.stress_level.toUpperCase()}</td>
          <td>${v.days_without_water}</td>
          <td>${hasActiveAssignment(v.id) ? '🚛 Assigned' : '⏳ Pending'}</td>
        </tr>
      `).join('')}
    </tbody>
  </table>
  ...
  <div class="footer">
    <p>Report generated by JalRakshak - AI-Powered Drought Monitoring System</p>
    <p>Data sources: IMD, CGWB, Census India, OpenStreetMap, Open-Meteo API</p>
  </div>
          ...
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="flex items-center space-x-3">
              <button
                ...
                className="bg-blue-600 text-white py-2 px-4 rounded-lg hover:bg-blue-700 transition font-semibold text-sm flex items-center"
              >
                📄 Export Report
              </button>
